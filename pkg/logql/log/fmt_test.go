...
func Test_labelsFormatter_Format(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			labels.FromStrings("bar", "19503", "foo", epochDay19503.String()),
		},
		{
			"unixToTime seconds",
			mustNewLabelsFormatter([]LabelFmt{NewTemplateLabelFmt("foo", `{{ .bar | unixToTime }}`)}),
			labels.FromStrings("foo", "", "bar", "1679577215"),
			labels.FromStrings("bar", "1679577215", "foo", epochSeconds1679577215.String()),
		},
		{
			"unixToTime milliseconds",
			mustNewLabelsFormatter([]LabelFmt{NewTemplateLabelFmt("foo", `{{ .bar | unixToTime }}`)}),
			labels.FromStrings("foo", "", "bar", "1257894000000"),
			labels.FromStrings("bar", "1257894000000", "foo", epochMilliseconds1257894000000.String()),
		},
		{
			"unixToTime microseconds", 
			mustNewLabelsFormatter([]LabelFmt{NewTemplateLabelFmt("foo", `{{ .bar | unixToTime }}`)}),
			labels.FromStrings("foo", "", "bar", "1673798889902000"),
			labels.FromStrings("bar", "1673798889902000", "foo", epochMicroseconds1673798889902000.String()),
		},
		{
			"unixToTime nanoseconds",
			mustNewLabelsFormatter([]LabelFmt{NewTemplateLabelFmt("foo", `{{ .bar | unixToTime }}`)}),
			labels.FromStrings("foo", "", "bar", "1000000000000000000"),
			labels.FromStrings("bar", "1000000000000000000", "foo", epochNanoseconds1000000000000000000.String()),
		},
	}
